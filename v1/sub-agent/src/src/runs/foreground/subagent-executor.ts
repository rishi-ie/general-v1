...
function appendStepToAsyncChain(input: {
  ...
  if (status.mode !== "chain") {
    return {
      content: [
        { type: "text", text: `Run '${resolved.id}' is ${status.mode}; only active chain runs accept appended steps.` },
      ],
      isError: true,
      details: { mode: "management", results: [] },
    };
  ...
  const execute = async (
    ...
    if (params.action) {
      ...
      if (deps.allowMutatingManagementActions === false && MUTATING_MANAGEMENT_ACTIONS.has(params.action)) {
        return {
          content: [
            { type: "text", text: `Action '${params.action}' is not available from child-safe subagent fanout mode.` },
          ],
          isError: true,
          details: { mode: "management" as const, results: [] },
        };
        ...
        hasTasks && effectiveParams.tasks
          ...
          : hasChain && effectiveParams.chain
            ? effectiveParams.chain.flatMap((step) =>
                isParallelStep(step) ? step.parallel.map((task) => task.agent) : [(step as SequentialStep).agent],
              )
