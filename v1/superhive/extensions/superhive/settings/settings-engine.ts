...
import { AgentRecord, type SettingsPatch, type ValidationError } from "../types";
...
export class SettingsEngine extends EventEmitter {
  ...
  async loadCurrentSettings(agentId: string): Promise<Record<string, unknown> | null> {
    ...
    const loaded = await this.store.read<Record<string, unknown>>(agentSettingsPath(this.store["dir"], agentId));
  ...
  private applyPatch(current: Record<string, unknown>, patch: SettingsPatch[]): Record<string, unknown> {
