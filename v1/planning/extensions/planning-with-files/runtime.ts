...
import { type PlanStatus, isAllPhasesComplete, isPlanIncomplete, isSessionAttached, readPlanStatus } from "./plan.ts";
...
function runAttestScript(cwd: string, args: string[]): ExecResult {
  ...
  if (process.platform === "win32" && existsSync(ATTEST_PS1)) {
    ...
    candidates.push(["pwsh", ["-NoProfile", "-ExecutionPolicy", "RemoteSigned", "-File", ATTEST_PS1, ...args]]);
  }
