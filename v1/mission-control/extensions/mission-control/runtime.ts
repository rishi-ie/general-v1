...
import { inferTasksFromConversation, filterByConfidence, formatAutoCaptureAnnouncement, formatAutoCaptureConfirmation } from './auto-capture.js';
...
function registerCommands(pi: ExtensionAPI, state: RuntimeState): void {
  pi.registerCommand(COMMANDS.NEW, {
    description: 'Create a new ticket: /ticket new <title> [--desc <description>] [--priority <level>] [--tags <tag1,tag2>]',
  ...
  pi.registerCommand(COMMANDS.UPDATE, {
    description: 'Update ticket fields: /ticket update <id> [--status <status>] [--priority <priority>] [--title <title>] [--desc <description>]',
    handler: async (args: string) => {
      const parts = args.trim().split(/\s+/);
      const id = parts[0];
      if (!id) return 'Usage: /ticket update <id> [--status <status>] [--priority <priority>] [--title <title>] [--desc <description>]';
